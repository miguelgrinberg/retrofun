...
def paginated_orders(start, length, sort, search):
    ...
    q = sa.select(Order, total).distinct().join(Order.customer).join(
        Order.order_items).join(OrderItem.product).group_by(Order)
...
def total_orders(search):
    ...
    return sa.select(sa.func.count(sa.distinct(Order.id))).join(
        Order.customer).join(Order.order_items).join(OrderItem.product).where(
            sa.or_(
                Customer.name.ilike(f'%{search}%'),
                Product.name.ilike(f'%{search}%'),
            )
